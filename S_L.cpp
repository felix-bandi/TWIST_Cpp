...
void MainWindow::Save(mentes ment)
{
	...
	if (f.is_open())
	{
		...
		for (size_t i = 0; i < POLIGON_vector.size(); i++)
		{
			...
			for (size_t p = 0; p < POLIGON_vector[i].pont.size(); p++)
			{
...
void MainWindow::Load(mentes tölt)
{
	...
	int alk_szam = -1;
	std::wstring wsor;
	char s[1000], *a=nullptr, *b=nullptr, ss[1000];
	ALKATRESZ alkatresz;
	...
	if (f.is_open())
	{
		...
		while (f.good())
		{
			...
			{
				alk_szam++;
				for (size_t i = 1; i < std::strlen(ss); i++) alkatresz.nev.push_back(ss[i]);
				save = 111;
			}
			if(s[0] == 'V')
			{
			...
			if (s[0] == 'P')
			{
				...
				for (size_t i = 0; i < poli.pont.size(); i++) pontok[i] = poli.pont[i];
			...
			if (alk && s[0]=='|')
			{
				if (VONAL_betolt.size() > 0) for (size_t i = 0; i < VONAL_betolt.size(); i++)     alkatresz.v.push_back(VONAL_betolt[i]);
				if (ARC_betolt.size() > 0)   for (size_t i = 0; i < ARC_betolt.size(); i++)       alkatresz.a.push_back(ARC_betolt[i]);
				...
				if (EL_betolt.size() > 0)    for (size_t i = 0; i < EL_betolt.size(); i++)    alkatresz.ep.push_back(EL_betolt[i]);
				if (SQ_betolt.size() > 0)    for (size_t i = 0; i < SQ_betolt.size(); i++)    alkatresz.sp.push_back(SQ_betolt[i]);
				if (RR_betolt.size() > 0)    for (size_t i = 0; i < RR_betolt.size(); i++)    alkatresz.rp.push_back(RR_betolt[i]);
				if (ELVIA_betolt.size() > 0) for (size_t i = 0; i < ELVIA_betolt.size(); i++) alkatresz.ev.push_back(ELVIA_betolt[i]);
				if (SQVIA_betolt.size() > 0) for (size_t i = 0; i < SQVIA_betolt.size(); i++) alkatresz.sv.push_back(SQVIA_betolt[i]);
				if (RRVIA_betolt.size() > 0) for (size_t i = 0; i < RRVIA_betolt.size(); i++) alkatresz.rv.push_back(RRVIA_betolt[i]);
	...
	if (pos != std::wstring::npos) root.erase(pos); // Projekt főkönyvtár

...
{
	int i = static_cast<int>(ARC.size() - 1);
	...
	if (majdnem(ak, av))
	{
		D2D1_POINT_2F start = D2D1::Point2F(ARC[i].kpx + ARC[i].rx, ARC[i].kpy);
		D2D1_POINT_2F mid = D2D1::Point2F(ARC[i].kpx - ARC[i].rx, ARC[i].kpy);
		D2D1_POINT_2F end = start;
		pSink->BeginFigure(ak, D2D1_FIGURE_BEGIN_HOLLOW);
		pSink->AddArc(D2D1::ArcSegment(start, meret, 0.0, ARC[i].i, ARC[i].s));
		pSink->AddArc(D2D1::ArcSegment(mid, meret, 0.0f, ARC[i].i, ARC[i].s));
		pSink->AddArc(D2D1::ArcSegment(end, meret, 0.0f, ARC[i].i, ARC[i].s));
	}
	else
	{
		pSink->BeginFigure(ak, D2D1_FIGURE_BEGIN_HOLLOW);
...
void MainWindow::ARC_rajzol_2() // FIXME: 2felkoros 
{
	...
	szt = (arc.i == D2D1_SWEEP_DIRECTION_CLOCKWISE) ? (arc.ksz - arc.vsz) : (arc.vsz - arc.ksz);
	if (szt < 0) szt += 2 * M_PI;
	...
	if (majdnem(ak, av))
	{
		D2D1_POINT_2F start = D2D1::Point2F(arc.kpx + arc.rx, arc.kpy);
		D2D1_POINT_2F mid = D2D1::Point2F(arc.kpx - arc.rx, arc.kpy);
		D2D1_POINT_2F end = start;
		pSink->BeginFigure(start, D2D1_FIGURE_BEGIN_HOLLOW);
		pSink->AddArc(D2D1::ArcSegment(mid, meret, 0.0f, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_LARGE));
		pSink->AddArc(D2D1::ArcSegment(end, meret, 0.0f, D2D1_SWEEP_DIRECTION_CLOCKWISE, D2D1_ARC_SIZE_LARGE));
	}
	else
	{
		pSink->BeginFigure(ak, D2D1_FIGURE_BEGIN_HOLLOW);
